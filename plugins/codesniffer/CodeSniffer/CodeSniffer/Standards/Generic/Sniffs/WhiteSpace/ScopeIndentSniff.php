...
class Generic_Sniffs_WhiteSpace_ScopeIndentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$firstToken]['code'] !== T_CLOSURE
            ...
            $error = _AM_XOOPSSECURE_CODESNIFFER_LII;
            $data  = array(
                      ($expectedIndent - 1),
            ...
            if ($newline === true && $tokens[$i]['code'] !== T_WHITESPACE) {
                ...
                if ($column !== $indent) {
                    if ($this->exact === true || $column < $indent) {
                        $type  = 'IncorrectExact';
                        $error = _AM_XOOPSSECURE_CODESNIFFER_LIIE;
                        if ($this->exact === false) {
                            $error .= _AM_XOOPSSECURE_CODESNIFFER_AL;
                            $type   = 'Incorrect';
                        }

                        $error .= _AM_XOOPSSECURE_CODESNIFFER_FOUNDSPACES;
                        $data = array(
                                  ($indent - 1),
                                  ($column - 1),
                                );
