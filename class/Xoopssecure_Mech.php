...
class Xoopssecure_Mech
{
     ...
     */
    public function testServer(): array
    {
        ...
        // core
        $allowurlfopen = $init['Core']['allow_url_fopen']['local'];
        // off
        $allowurlinclude = $init['Core']['allow_url_include']['local'];
        // off
        $registerglobals = ($phpversion < '5.3.0') ? $init['Core']['register_globals']['local'] : 'Off';
        // off
        $openbasedir = $init['Core']['open_basedir']['local'];
        // "c:\inetpub\"
        $safemode = ($phpversion < '5.3.0') ? $init['Core']['safe_mode']['local'] : 'Off';
        // off
        $safemodegid = ($phpversion < '5.3.0') ? $init['Core']['safe_mode_gid']['local'] : 'Off';
        // off
        $maxexecutiontime = $init['Core']['max_execution_time']['local'];
        // < 30
        $maxinputtime = $init['Core']['max_input_time']['local'];
        // < 60
        $memorylimit = $this->convertToBytes($init['Core']['memory_limit']['local']);
        // < 16M
        $uploadmaxfilesize = $init['Core']['upload_max_filesize']['local'];
        // <2M
        $postmaxsize = $init['Core']['post_max_size']['local'];
        // < 8M
        $maxinputnestinglevels = $init['Core']['max_input_nesting_level']['local'];
        // <32
        $displayerrors = $init['Core']['display_errors']['local'];
        // Off
        $logerrors = $init['Core']['log_errors']['local'];
        // On
        $errorlog = $init['Core']['error_log']['local'];
        // "C:\path\of\your\choice"
        $exposephp = $init['Core']['expose_php']['local'];
        // Off
        $enabledl = $init['Core']['enable_dl']['local'];
        // Off
        $disablefunc = $init['Core']['disable_functions']['local'];
        ...
        if ($allowurlfopen == 'On') {
            ...
            $resp['phpini'][$i]['errortype']   = $this->errorButton('warning', $resp['phpini'][$i]['ref']);
        }
     ...
     */
    public function mysqlUptimeToString($seconds): string
    {
     ...
     * @return boolean
     */
    public function isLocalhost($whitelist=['127.0.0.1', '::1']): bool
    {
     ...
     * @return float|integer|the $n bytes
     */
    public function convertToBytes($n): float|int|the
    {
        ...
        return match (substr($n, -1)) {
            'B' => substr($n, 0, -1),
            'K' => (substr($n, 0, -1) * 1024),
            'M' => (substr($n, 0, -1) * 1024 * 1024),
            'G' => (substr($n, 0, -1) * 1024 * 1024 * 1024),
            default => $n,
        };
    }//end convertToBytes()
     ...
     * @return array $resp
     */
    public function systemArray(): array
    {
        $resp            = [];
     ...
     */
    public function getApacheModules($val): array
    {
        ...
        return $trn;
    }//end getApacheModules()


    /**
     * parse php modules from phpinfo
     *
     * @param string $name Which part of info to get. Possible names are =
     ...
     * @return array $config
     */
    public function parsePHPModules($name): array
    {
        ob_start();
        phpinfo($name);
        $s = ob_get_contents();
     ...
     * @return json|array   $result   Potential issues
     */


    public function getVul($software, $ver, $severe): json|array
    {
        $result = [];
        switch ($software) {
            case 'php':
        ...
        $postURL = 'http://www.cvedetails.com/json-feed.php?'.'numrows=10&'.'cvssscoremin='.$severe.'&'.'vendor_id='.$vendor_id.'&product_id='.$product_id.'&version_id='.$version_id;
     ...
     * @param  string  $soft
     * @param  integer $ver
     * @return integer|string $resp
     */
    public function getNumCVE($soft, $ver): int|string
    {
        $resp = [
            'xoops'  => [
                '2.5.10'  => 353048,
     ...
     * @return string|array|null $resp unix date of release
     */
    public function mysqlVersion($ver): string|array|null
    {
        $version = [
            '8.0.31'  => '1665439200',
            '8.0.30'  => '1657058400',
     ...
     * @return string|array|null $resp  unix date of release
     */
    public function mariadbVersion($ver): string|array|null
    {
        $version = [
            '10.9.04'  => '1667775600',
            '10.9.03'  => '1663538400',
     ...
     * @return string|array|null $resp  unix date of release
     */
    public function apacheVersion($ver): string|array|null
    {
        $version = [
     ...
     * @return string|array|null $version
     */
    public function xoopsVersion($ver): string|array|null
    {
        $version = [
            '2.5.112' => '1659477600',
            '2.5.111' => '1581634800',
