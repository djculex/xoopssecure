<?php
/**
 * Xoopssecure_FileH.php class file
 *
 * @package    xoopssecure
 * @sub-packet Xoops 2.5.10
 * @author     Culex <culex@culex.dk>
 * @copyright 2015-2023 culex (www.culex.dk)
 */
 ...
 */
class Xoopssecure_FileH extends XoopsPersistableObjectHandler
{

    /**
     * Timestamp is unix time from where scan begin
     * @var int|string $timestamp
     */
    public int|string $timestamp;

    /**
     * Array of files to present in json.
     * @var array $FileDir
     */
    public array $FileDir;

    /**
     * From where to start scannning.
     * @var array|mixed $startPath
     */
    public mixed $startPath;

    /**
     * How long in days to keep back-ups.
     * @var integer $deleteBackupAfterDays
     */
    public int $deleteBackupAfterDays;

    /**
     * How many days between each back up.
     * @var false|int $backupFilesMaxAge
     */
    public int|false $backupFilesMaxAge;

    /**
     * Is it time now for back up ? true or false.
     * @var bool|string $timeForBackup
     */
    public string|bool $timeForBackup;

    /**
     * Do we run cron file now ? true or false.
     * @var bool|string $timeForCron
     */
    ...
    /**
     * The helper.
     * @var Xoopssecure_Helper|null $helper
     */
    public ?Xoopssecure_Helper $helper;
        ...
        if (!empty($files)) {
            ...
            foreach ($files as $file) {
                echo "<tr id='xoopssecure_trbackup_".$file['filename']."'>
								...
								<td class='size'>
                                    <a href=".XOOPS_URL.'/uploads/backup/'.$file['filename'].'>'.DO_XOOPSSECURE_DOWNLOAD_BACKUPFILEDOWNLOADTEXT."
                                    </a>
                                </td>
								<td class='delete'>
									<a role='button' 
            ...
            echo "<tr id='xoopssecure_trbackup_0'>
							<td class='type'></td>
							<td colspan='4' class='name truncate' style='font-size: 16px;text-align: center;font-weight: bold;'>".DO_XOOPSSECURE_NOBACKUPYET."
                            </td>
							<td class='delete'></td>
						</tr>";
        }//end if
    ...
    {
        global $dir_paths;
        // global variable where to store the result
        foreach ($path as $dir) {
            // loop the input
            $dir_paths[] = str_replace('\\', '/', $dir);
            // can use also "basename($dir)" or "realpath($dir)"
            $subdir = glob($dir.DIRECTORY_SEPARATOR.'*', GLOB_ONLYDIR);
            // use DIRECTORY_SEPARATOR to be OS independent
            if (!empty($subdir)) {
                // if subdir is not empty make function recursive
                $this->listdirs($subdir);
                // execute the function again with current subdir
            }
        }
    ...
    {
        if ($_SESSION['xoopssecureCoreEvents'] == 0) {
            // Check $_session
            global $xoopsConfig;
            $theme = new XoTheme();
            // Set javascript vars
    ...
    public function cronScan(): void
    {
        if ((int) $this->helper->getConfig('XCISCRONTYPE') == 1) {
            if ($this->timeForCron === true) {
                ...
                $checkinterval = (int) $this->helper->getConfig('XCISCRONINTERVAL');
                // 24
     ...
     *
     * @param  $info
     * @return boolean according to success of send.
     */
    public function sendCronMail($info): bool
    {
        ...
        if ($this->timeForCron === true) {
            ...
            $lnk = XOOPS_ROOT_PATH.'/modules/xoopsSecure/language/'.$GLOBALS['xoopsConfig']['language'].'/mail/mail_cron.tpl';
            ...
            $xoopsMailer->setTemplateDir(
                XOOPS_ROOT_PATH.'/modules/xoopssecure/language/'.$GLOBALS['xoopsConfig']['language'].'/mail/'
            );
