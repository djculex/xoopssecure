...
class Tokenizer
{
     ...
     * @throws Exception
     */
    private function _getAllTokens($source): array
    {
        $newTokens = [];
    ...
    private function _identifyTokens($tokenText, $tokenID)
    {
        ...
        $splitData = preg_split('#(\r\n|\n|\r)#', $tokenText, 0, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
    ...
    public function findClosingParenthesisPosition($startPos)
    {
        ...
        while ($parenthesisCount > 0 && $pos < $nbTokens) {
            ...
            if ($token->id === T_PARENTHESIS_OPEN) {
                $parenthesisCount += 1;
            } elseif ($token->id === T_PARENTHESIS_CLOSE) {
                $parenthesisCount -= 1;
            }
