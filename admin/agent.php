...
switch ($type) {
    ...
    case 'getSourceCode':
        ...
        $g = new Geshi();
        // Initiate GeSHi class
        $fxt = pathinfo($fn, PATHINFO_EXTENSION);
        // Get extension of file
        $g->set_language(strtoupper($fxt));
        // CAP LETTERS of ext.
        $g->load_from_file($fn);
        // Log content of file
        $g->set_header_type(GESHI_HEADER_PRE_VALID);
        // Make a header
        $g->enable_classes(true);
        // Enable classes
        $g->enable_line_numbers(GESHI_FANCY_LINE_NUMBERS, 37);
        // Enable line numbers
        $g->set_overall_style('color: #000066; border: 1px solid #d0d0d0; background-color: #f0f0f0;', true);
        // Set overall style DUH
        ...
        // ditto again
        if ($ln != '' || $ln != 0) {
            $g->highlight_lines_extra((int) $ln, 'background-color:yellow');
            // if line number is real make highligh yellow of this line
        }

        echo '<style>';
        // Echo style start
        echo $g->get_stylesheet();
        // echo stylesheet for file ext
        echo '</style>';
        // close style
        echo $g->parse_code();
        // print the styles, numbered and highlighted code
    break;
    ...
    case 'singleFileTest':
        $dir = $_GET['Dir'];
        // dir
        $autoFeature = $_GET['checkIndexfiles'];
        // bool
        $scanstart = $_GET['scanstart'];
        // timestamp
